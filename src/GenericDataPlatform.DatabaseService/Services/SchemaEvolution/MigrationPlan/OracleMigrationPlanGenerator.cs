...
{
    public class OracleMigrationPlanGenerator : BaseMigrationPlanGenerator
    {
        public OracleMigrationPlanGenerator(SchemaComparer schemaComparer, ILogger<OracleMigrationPlanGenerator> logger)
            ...
            List<SchemaChange> changes)
        {
            ...
            if (addFieldChanges.Any())
            {
                ...
                foreach (var change in addFieldChanges)
                {
                    ...
                    // Oracle uses ALTER TABLE ADD for adding columns
                    sql.AppendLine($"ALTER TABLE {tableName} ADD {fieldName} {GetSqlTypeForFieldType(fieldType, DatabaseType.Oracle)}");
                    ...
                    if (isRequired && !string.IsNullOrEmpty(defaultValue))
                    {
                        ...
                        sql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} DEFAULT '{defaultValue}' NOT NULL");
                    }
                    else if (isRequired)
                    {
                        ...
                        sql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} NOT NULL");
                    }
                    else if (!string.IsNullOrEmpty(defaultValue))
                    {
                        sql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} DEFAULT '{defaultValue}'");
                    }
            ...
            if (removeFieldChanges.Any())
            {
                ...
                foreach (var change in removeFieldChanges)
                {
                    ...
                    rollbackSql.AppendLine($"ALTER TABLE {tableName} ADD {fieldName} {GetSqlTypeForFieldType(fieldType, DatabaseType.Oracle)}");
                    
                    if (isRequired && !string.IsNullOrEmpty(defaultValue))
                    {
                        rollbackSql.AppendLine($"UPDATE {tableName} SET {fieldName} = '{defaultValue}' WHERE {fieldName} IS NULL");
                        rollbackSql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} DEFAULT '{defaultValue}' NOT NULL");
                    }
                    else if (isRequired)
                    {
                        ...
                        rollbackSql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} NOT NULL");
                    }
                    else if (!string.IsNullOrEmpty(defaultValue))
                    {
                        rollbackSql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} DEFAULT '{defaultValue}'");
                    }
                }
            ...
            if (renameFieldChanges.Any())
            {
                ...
                foreach (var change in renameFieldChanges)
                {
                    ...
                    // Oracle uses ALTER TABLE RENAME COLUMN for renaming columns
            ...
            if (changeTypeChanges.Any())
            {
                ...
                foreach (var change in changeTypeChanges)
                {
                    ...
                    var oldSqlType = GetSqlTypeForFieldType(oldType, DatabaseType.Oracle);
                    var newSqlType = GetSqlTypeForFieldType(newType, DatabaseType.Oracle);
                    
                    // Oracle uses ALTER TABLE MODIFY for changing column types
                    sql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} {newSqlType}");
                    rollbackSql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} {oldSqlType}");
                }
            ...
            if (changeRequiredChanges.Any())
            {
                ...
                foreach (var change in changeRequiredChanges)
                {
                    ...
                    if (newRequired && !oldRequired)
                    {
                        ...
                        rollbackSql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} NULL");
                    }
                    else if (!newRequired && oldRequired)
                    {
                        // Field is no longer required
                        sql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} NULL");
            ...
            if (changeDefaultChanges.Any())
            {
                ...
                foreach (var change in changeDefaultChanges)
                {
                    ...
                    {
                        sql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} DEFAULT '{newDefault}'");
                    }
                    else
                    {
                        sql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} DEFAULT NULL");
                    }
                    
                    if (!string.IsNullOrEmpty(oldDefault))
                    {
                        rollbackSql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} DEFAULT '{oldDefault}'");
                    }
                    else
                    {
                        rollbackSql.AppendLine($"ALTER TABLE {tableName} MODIFY {fieldName} DEFAULT NULL");
                    }
                }
            ...
            List<SchemaChange> changes)
        {
            ...
            foreach (var change in renameChanges)
            {
                ...
                // For Oracle, we use RENAME COLUMN which doesn't require data copying
        ...
        private string GenerateTypeConversionScript(string tableName, string fieldName, FieldType fromType, FieldType toType)
        {
            ...
            switch (toType)
            {
                case FieldType.String:
                    ...
                    script.AppendLine($"UPDATE {tableName} SET {fieldName} = TO_CHAR({fieldName}) WHERE {fieldName} IS NOT NULL");
                    break;
                ...
                case FieldType.Integer:
                    if (fromType == FieldType.String || fromType == FieldType.Decimal || fromType == FieldType.Boolean)
                    {
                        ...
                        script.AppendLine($"    WHEN REGEXP_LIKE({fieldName}, '^[0-9]+$') THEN TO_NUMBER({fieldName})");
                ...
                case FieldType.Decimal:
                    ...
                    {
                        script.AppendLine($"UPDATE {tableName} SET {fieldName} = CASE");
                        script.AppendLine($"    WHEN REGEXP_LIKE({fieldName}, '^[0-9]+(\\.[0-9]+)?$') THEN TO_NUMBER({fieldName})");
                        script.AppendLine($"    ELSE NULL");
                        script.AppendLine($"END WHERE {fieldName} IS NOT NULL");
                    }
                    break;
                ...
                case FieldType.DateTime:
                    ...
                    {
                        script.AppendLine($"UPDATE {tableName} SET {fieldName} = CASE");
                        script.AppendLine($"    WHEN REGEXP_LIKE({fieldName}, '^[0-9]{{4}}-[0-9]{{2}}-[0-9]{{2}}.*') THEN TO_TIMESTAMP({fieldName}, 'YYYY-MM-DD\"T\"HH24:MI:SS')");
                        script.AppendLine($"    ELSE NULL");
                        script.AppendLine($"END WHERE {fieldName} IS NOT NULL");
                    }
                    break;
