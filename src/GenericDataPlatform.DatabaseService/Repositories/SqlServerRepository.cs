...
namespace GenericDataPlatform.DatabaseService.Repositories
{
    public class SqlServerRepository : IDbRepository
    {
        ...
        public SqlServerRepository(IOptions<DatabaseOptions> options, ILogger<SqlServerRepository> logger)
        {
        ...
        public async Task<bool> CreateTableAsync(string sourceId, DataSchema schema)
        {
            try
            {
                ...
                var tableExists = await connection.ExecuteScalarAsync<int>(tableExistsQuery, new { tableName }) > 0;
        ...
        {
            return new SqlConnection(_options.SqlServerConnectionString);
        }
        ...
        private async Task EnsureDataRecordsTableExistsAsync(IDbConnection connection)
        {
            ...
            var tableExistsQuery = @"
                SELECT COUNT(*)
                FROM sys.tables
                WHERE name = 'data_records'";
            
            var tableExists = await connection.ExecuteScalarAsync<int>(tableExistsQuery) > 0;
