...
namespace GenericDataPlatform.ML.Training
{
    ...
    public class ModelTrainer : IModelTrainer
    {
        ...
        public ModelTrainer(
            ...
            IDynamicObjectGenerator dynamicObjectGenerator = null)
        {
        ...
        {
            if (_dynamicObjectGenerator != null)
            {
                // Use the dynamic object generator to create a type
                return _dynamicObjectGenerator.CreateType(modelDefinition.Features, modelDefinition.Labels);
            }
            else
            {
                _logger.LogWarning("DynamicObjectGenerator not available, using Dictionary<string, object> as fallback");
                // Fallback to Dictionary if the generator is not available
    ...
    /// <summary>
    /// Implementation of the overloaded TrainModelAsync method that includes validation data and context
    /// </summary>
    public async Task<TrainedModel> TrainModelAsync(
        ModelDefinition modelDefinition,
        ...
        TrainingContext context)
    {
        try
        {
            _logger.LogInformation("Training model: {ModelName} with job ID: {JobId}",
                modelDefinition.Name, context?.JobId);

            // Convert training data to IDataView
            var trainingDataView = ConvertToDataView(trainingData, modelDefinition);

            // Convert validation data if provided
            IDataView validationDataView = null;
            if (validationData != null && validationData.Any())
            {
                validationDataView = ConvertToDataView(validationData, modelDefinition);
            }

            // Use validation data if provided, otherwise split the training data
            IDataView trainSet, testSet;
            if (validationDataView != null)
            {
                trainSet = trainingDataView;
                testSet = validationDataView;
            }
            else
            {
                var dataSplit = _mlContext.Data.TrainTestSplit(trainingDataView, testFraction: 0.2);
                trainSet = dataSplit.TrainSet;
                testSet = dataSplit.TestSet;
            }
            ...
            {
                case ModelType.BinaryClassification:
                    trainedModel = TrainBinaryClassificationModel(modelDefinition, trainSet);
                    break;

                case ModelType.MultiClassClassification:
                    trainedModel = TrainMultiClassClassificationModel(modelDefinition, trainSet);
                    break;

                case ModelType.Regression:
                    trainedModel = TrainRegressionModel(modelDefinition, trainSet);
                    break;

                case ModelType.Clustering:
                    trainedModel = TrainClusteringModel(modelDefinition, trainSet);
                    break;

                case ModelType.AnomalyDetection:
                    trainedModel = TrainAnomalyDetectionModel(modelDefinition, trainSet);
                    break;

                case ModelType.Recommendation:
                    trainedModel = TrainRecommendationModel(modelDefinition, trainSet);
                    break;

                case ModelType.TimeSeries:
                    trainedModel = TrainTimeSeriesModel(modelDefinition, trainSet);
                    break;
            ...
            // Evaluate the model
            var metrics = EvaluateModel(modelDefinition, trainedModel, testSet);
            ...
            _logger.LogInformation("Model training completed: {ModelName} with job ID: {JobId}",
                modelDefinition.Name, context?.JobId);
        ...
        {
            _logger.LogError(ex, "Error training model: {ModelName} with job ID: {JobId}",
                modelDefinition.Name, context?.JobId);
            throw;
        }
    }
